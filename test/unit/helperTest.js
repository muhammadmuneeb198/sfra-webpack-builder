...
describe('buildRuleSet()', () => {
    it('should use eslint when linter is set to true', () => {
        let env = {};
        env.useLinter = true;
        const rules = WebpackHelper.buildRuleSet(__dirname, path.resolve(__dirname, '../fixtures'), env, 'js');
        let hasLinter = false;
        rules.map((rule) => {
    ...
    it('should not use eslint when linter is set to true', () => {
        let env = {};
        env.useLinter = false;
        const rules = WebpackHelper.buildRuleSet(__dirname, path.resolve(__dirname, '../fixtures'), env, 'js');
        let hasLinter = false;
    ...
    it('scss should have source-maps if env is set to dev', () => {
        let env = {};
        env.dev = true;
        const rules = WebpackHelper.buildRuleSet(__dirname, path.resolve(__dirname, '../fixtures'), env, 'scss');
