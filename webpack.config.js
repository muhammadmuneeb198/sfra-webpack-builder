...
class WebpackBundle {
    ...
    static scanEntryPoints(cartridge, fileType) {
        const srcPath = path.resolve(process.env.PWD, cartridge, 'cartridge/client');
    ...
    */
    static bundleCartridge(env = {}, cartridge, fileType) {
        ...
        if (Object.keys(entryFiles).length === 0) {
            console.error(`Entry not found - please check if ${fileType} folder exist in your cartridge : ${cartridge}`);
            return {};
        }
        ...
        const outputPath = path.resolve(process.env.PWD, cartridge, 'cartridge', 'static');
...
module.exports = env => {
    ...
    return bundlesFiles.filter(bundleFiles => Object.keys(bundleFiles).length > 0);
};
